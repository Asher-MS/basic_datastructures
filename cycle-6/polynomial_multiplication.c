...
void add_poly(struct node** head1,struct node** head2,struct node** result)
{
...
void mult_poly(struct node** head1,struct node** head2)
{
	...
	{	
		...
		while(temp2!=NULL)
		{
			struct node* newnode=(struct node*)malloc(sizeof(struct node));
			newnode->link=NULL;
			newnode->e=temp1->e+temp2->e;
			newnode->c=temp2->c*temp2->c;
			struct node* temp=NULL;
			printf("newnode : ");
			display(&newnode);
			printf("result : ");
			display(&result);
			add_poly(&result,&newnode,&temp);
			result=temp;
